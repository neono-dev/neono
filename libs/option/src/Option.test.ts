...
describe("Option", () => {
  ...
  describe("with a `Some` option", () => {
    ...
    it("`transpose` should return a Result.Ok", () => {
      const option: Option.Option<Result.Result<number, string>> = Option.Some(Result.Ok(5))
    ...
    it("`transpose` should return a Result.Err", () => {
      const option: Option.Option<Result.Result<number, string>> = Option.Some(Result.Err("Oh no"))
      const result = Option.transpose(option)
